...
variable [CompleteBooleanAlgebra l] [MPropOrdered m l] [LawfulMonad m] [MPropDetertministic m l]
...
example (n : ℕ) : ⊤ <= (foo n).prop (fun _ => ⊤) := by
...
def spmv (x_ind : Array (Array ℕ)) (x_val : Array (Array ℤ)) : NonDetT (StateT (Array ℤ) Id) Unit := do
  let mut arrInd : Array ℕ := Array.mkArray x_ind.size 0
