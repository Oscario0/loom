...
private structure ExtractMonadResult where
  m            : Expr
  returnType   : Expr
  expectedType : Expr
...
/--
We use `MProdWithNames` instead of `Prod` to group values when expanding the
`do` notation. `MProdWithNames` is a universe monomorphic product.
...
actions to be in the same universe.
-/
private def mkTuplePat (elems : Array Syntax) : MacroM Syntax := do
  if elems.size = 0 then
    mkUnit
  else if h : elems.size = 1 then
    return elems[0]
